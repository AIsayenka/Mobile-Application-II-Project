...
#import <xAPIKit/EXPStatementQueuePersisting.h>
 ...
 */
@interface EXPStatementQueue : NSObject
 ...
 */
@property (nonatomic, strong) id<EXPStatementQueuePersisting> persistenceCoordinator;

/**
 An array of statements in the queue that have not been sent to the LRS for persistence.
 The EXPAPI statement posting interval uses this value to POST batches of statements to the LRS.
 ...
 */
@property (weak, nonatomic, readonly) NSArray *unsentStatements;
 ...
 */
- (void) addStatement:(EXPStatement *)statement;
 ...
 */
- (NSArray *) getQueuedStatements:(NSUInteger)count;
...
/**
 ...
 @return        An array of queued statements ordered by oldest to newest queued.
 */
- (NSArray *) getQueuedStatements:(NSUInteger)count startingAtIndex:(NSUInteger)index;
 ...
 */
- (void) removeStatement:(EXPStatement *)statement;
 ...
 */
+ (EXPStatementQueue *) defaultStatementQueue;
