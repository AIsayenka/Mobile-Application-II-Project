...
/**
 Classes implementing the EXPAPIQueueDelegate protocol are eligible for receiving delegate callbacks
 from a EXPAPI instance about the status of statements it has in its queue.
 ...
 Typically, most implementations will only have one instance of EXPAPI (the defaultAPI) which means
 that most implementations will only have one statement queue. As such, there can only be one EXPAPIQueueDelegate.
 ...
 the EXPStatementsPersistedNotification.
 */
@protocol EXPAPIQueueDelegate <NSObject>
@optional
/**
 ...
 @remarks           After the delegate method is invoked, a notification named EXPStatementsPersistedNotification
                    is posted to the notification center. Refer to the EXPAPI docs for the contents of the userInfo dictionary.
 */
...
/**
 ...
 @param error       The error encountered when attempting to persist the statements to the LRS.
                    ...
                    You can enable niceErrorMessages on each EXPAPIRequest instance to return cleaner error
 ...
 @remarks           After the delegate method is invoked, a notification named EXPStatementsFailedPersistingNotification
                    is posted to the notification center. Refer to the EXPAPI docs for the contents of the userInfo dictionary.
*/
